...
class HierarchyNode extends Component {
    ...
    render() {
        return (
            <div className="node">
                <div className="node-head" onClick={this.handleClick}><img alt="icon" className={'arrow' + (this.state.expanded ? ' rotated' : '') + (this.props.childs ? '' : ' hidden')} src="/icons/chevron-right-solid.svg"/><img alt="icon" className="icon" src={this.props.icon}/> {this.props.name}</div>
